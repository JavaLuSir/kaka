export const onRequest: PagesFunction = async (context) => {
  ...
  if (path === "/" || path === "/index.html") {
    ...
    const data = await env.WEIGHT_KV.get("weight_records", "json");
...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .date-input { width: 100%; padding: 12px; border: 1px solid #ddd; border-radius: 8px; font-size: 16px; background: white; }
        ...
        .record-item { background: white; padding: 16px; margin-bottom: 12px; border-radius: 8px; display: flex; justify-content: space-between; align-items: center; }
        ...
        .delete-btn { background: #ff4757; padding: 6px 12px; font-size: 14px; width: auto; }
        .delete-btn:hover { background: #ff3344; }
        .chart-container { position: relative; height: 300px; margin-bottom: 20px; }
        .pagination { display: flex; justify-content: center; gap: 8px; margin-top: 20px; }
        .pagination button { width: auto; padding: 8px 14px; background: #667eea; }
        .pagination button:hover { background: #5568d3; }
        .pagination button:disabled { background: #ccc; cursor: not-allowed; }
        .pagination .page-info { display: flex; align-items: center; color: #666; font-size: 14px; }
        .stat-card { display: flex; justify-content: space-around; margin-bottom: 20px; }
...
<body>
    ...
    <div class="card">
        <h2>📊 数据统计</h2>
        <div class="stat-card">
            <div class="stat-item"><div class="stat-value" id="totalCount">-</div><div class="stat-label">总记录数</div></div>
            <div class="stat-item"><div class="stat-value" id="avgWeight">-</div><div class="stat-label">平均体重 (kg)</div></div>
            <div class="stat-item"><div class="stat-value" id="latestWeight">-</div><div class="stat-label">最新体重 (kg)</div></div>
        </div>
    </div>
    <div class="records">
        <h2>📋 记录历史</h2>
    ...
    <script>
        ...
        flatpickr("#date", { locale: "zh", dateFormat: "Y-m-d", defaultDate: new Date() });

        function loadRecords(page = 1) {
            currentPage = page;
            fetch(API_BASE + '?page=' + page).then(r => r.json()).then(data => {
                ...
                list.innerHTML = data.records.map(r => \`
                    <div class="record-item">
                        <div><div class="record-date">\${r.date}</div><div class="record-time">\${r.time}</div></div>
                ...
                renderPagination();
                updateStats(data);
                fetch(API_BASE + '?page=1&per_page=1000').then(r => r.json()).then(allData => {
                    if (allData.records) {
        ...
        function updateChart(data) {
            ...
            chart = new Chart(ctx, {
                ...
                data: {
                    labels: labels,
                    datasets: [{ label: '体重 (kg)', data: weights, borderColor: '#4CAF50', backgroundColor: 'rgba(76, 175, 80, 0.1)', fill: true, tension: 0.3, pointRadius: 5, pointBackgroundColor: '#4CAF50' }]
                },
                options: { responsive: true, maintainAspectRatio: false, scales: { y: { beginAtZero: false, title: { display: true, text: '体重 (kg)' } } } }
            });
        }
        ...
        function addRecord() {
            ...
            fetch(API_BASE, { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({date, weight}) })
                .then(r => r.json()).then(data => {
                    if (data.success) { document.getElementById('weight').value = ''; loadRecords(1); }
                });
        }

        function deleteRecord(id) {
            if (!confirm('确定删除这条记录？')) return;
            fetch(API_BASE + '?id=' + id, {method: 'DELETE'}).then(r => r.json()).then(data => {
